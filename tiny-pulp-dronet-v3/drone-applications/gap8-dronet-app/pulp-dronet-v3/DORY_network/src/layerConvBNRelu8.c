...
void layerConvBNRelu8(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 512;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+2048;
    copy_k.loc = (uint32_t) l1_buffer + 32876;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 512;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+2560;
    copy_lambda.loc = (uint32_t) l1_buffer + 33392;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  20000, // size: dimension of the buffer
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 30824) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+2048 + 512*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 32876 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+2560 + 512*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 33392 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 32876 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 33392 + exec_db_act);
    W = (char *) (l1_buffer + 30824 + exec_db_W);
    y = (char *) (l1_buffer + 20004 + db_y);
