...
// y_tile_size_h_last             7
// y_tile_size_w_last             7
...
// compute double buffering offsets and update db state
  ...
  for(iter=0; iter<1*1*1; iter++) {
    ...
    if(iter<1*1*1-1) 
    {
      y_tile_size_h   = (last_h_load)   ? 7 : 7;
      y_tile_size_w   = (last_w_load)   ? 7 : 7;
    }
    x = (char *) (l1_buffer + 0 + exec_db_x);
    x2 = (char *) (l1_buffer + 12552 + exec_db_x);
    ...
    y_tile_size_h   = (last_h_exec)   ? 7 : 7;
    y_tile_size_w   = (last_w_exec)   ? 7 : 7;
