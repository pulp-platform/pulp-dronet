...
void layerConvBNRelu6(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+18432;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+18944;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<2*1*2*4-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 10 : 17;
      x_tile_size_w   = (_i_w_load+1 == 4)   ? 2 : 9;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 32 : 32;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 9, 32, 25, 32,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+18432 + 256*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+18944 + 256*_i_nof_load;
    ...
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 10 : 17;
    x_tile_size_w_exec   = (_i_w_exec+1 == 4)   ? 2 : 9;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 32 : 32;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 32 : 32;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<2*1*2*4-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 4, 32, 13, 64, 0, 0, 0, 0, 0, 0, 8), // ext
