...
void layerConvBNRelu10(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+73728;
    copy_k.loc = (uint32_t) l1_buffer + 24652;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 64;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+74752;
    copy_lambda.loc = (uint32_t) l1_buffer + 24784;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  7488, // size: dimension of the buffer
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 15432) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<16*1*1*2; iter++) {
    ...
    db_x = !db_state_x ? 7488 : 0;
    db_W = !db_state_W ? 4608 : 0;
    db_y = !db_state_y ? 224 : 0;
    db_act = !db_state_W ? 64 : 0;
    exec_db_x = db_state_x ? 7488 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<16*1*1*2-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 13 : 13;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 6 : 9;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 3 : 4;
      W_tile_size_nof = (_i_nof_load+1 == 16) ? 8 : 8;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 13, 9, 64, 13, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 15432) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+73728 + 64*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 24652 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+74752 + 64*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 24784 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 24652 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 24784 + exec_db_act);
    W = (char *) (l1_buffer + 15432 + exec_db_W);
    y = (char *) (l1_buffer + 14980 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 6 : 9;
    y_tile_size_nof = (_i_nof_exec+1 == 16) ? 8 : 8;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 7 : 7;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 3 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 16)   ? 8 : 8;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<16*1*1*2-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 7, 4, 8, 7, 128, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 14980) + db_y, // loc
