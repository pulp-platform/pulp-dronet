...
void layerConvBNRelu12(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 1024;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+8192;
    copy_k.loc = (uint32_t) l1_buffer + 25292;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 1024;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+9216;
    copy_lambda.loc = (uint32_t) l1_buffer + 26320;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 17096) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_act = !db_state_W ? 1024 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 8192 : 0;
    exec_db_act = db_state_W ? 1024 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 128, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 17096) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+8192 + 1024*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 25292 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+9216 + 1024*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 26320 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 25292 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 26320 + exec_db_act);
    W = (char *) (l1_buffer + 17096 + exec_db_W);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 128 : 128;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 128 : 128;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 7, 7, 128, 7, 128, 0, 0, 0, 0, 0, 0, 8), // ext
