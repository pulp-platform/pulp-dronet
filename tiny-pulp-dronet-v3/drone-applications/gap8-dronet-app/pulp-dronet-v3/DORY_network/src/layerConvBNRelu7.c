...
void layerConvBNRelu7(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+36864;
    copy_k.loc = (uint32_t) l1_buffer + 27148;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 128;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+37376;
    copy_lambda.loc = (uint32_t) l1_buffer + 27408;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 8712) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<4*1*2*4-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 6 : 10;
      x_tile_size_w   = (_i_w_load+1 == 4)   ? 2 : 6;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 5 : 8;
      y_tile_size_w   = (_i_w_load+1 == 4)   ? 1 : 4;
      W_tile_size_nof = (_i_nof_load+1 == 4) ? 16 : 16;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 6, 64, 13, 64,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 8712) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+36864 + 128*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 27148 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+37376 + 128*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 27408 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 27148 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 27408 + exec_db_act);
    W = (char *) (l1_buffer + 8712 + exec_db_W);
    ...
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 6 : 10;
    x_tile_size_w_exec   = (_i_w_exec+1 == 4)   ? 2 : 6;
    y_tile_size_nof = (_i_nof_exec+1 == 4) ? 16 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 5 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 4)   ? 1 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 4)   ? 16 : 16;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<4*1*2*4-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 4, 16, 13, 64, 0, 0, 0, 0, 0, 0, 8), // ext
