...
void layerMatMul14_last(
  ...
  6272, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  6272, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  1,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  6272, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 6288) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  12544, // size: dimension of matrix of weight * bytes_per_weight
  6272, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  6272, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_W = !db_state_W ? 12544 : 0;
    db_y = !db_state_y ? 8 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 12544 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 6272 : 6272;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 6272 : 6272;
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 2, 1*1, 6272, 1*1, 6272, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 6288) + db_W, // loc
    ...
    W = (char *) (l1_buffer + 6288 + exec_db_W);
    y = (char *) (l1_buffer + 6276 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 6272 : 6272;
