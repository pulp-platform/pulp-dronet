...
void layerConvBNRelu3(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 31244;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 31760;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 12808) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<1*1*4*3-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 2 : 10;
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 6 : 12;
      ...
      y_tile_size_w   = (_i_w_load+1 == 3)   ? 5 : 10;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 12, 32, 25, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.loc = (uint32_t) l1_buffer + 31244 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          ...
          copy_lambda.loc = (uint32_t) l1_buffer + 31760 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 31244 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 31760 + exec_db_act);
    W = (char *) (l1_buffer + 12808 + exec_db_W);
    ...
    x_tile_size_h_exec   = (_i_h_exec+1 == 4)   ? 2 : 10;
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 6 : 12;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 4)   ? 1 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 3)   ? 5 : 10;
    ...
    if(iter<1*1*4*3-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 10, 32, 25, 32, 0, 0, 0, 0, 0, 0, 8), // ext
