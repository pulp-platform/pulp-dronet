...
void layerConvBNRelu13(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+256;
    copy_k.loc = (uint32_t) l1_buffer + 1836;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 128;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+384;
    copy_lambda.loc = (uint32_t) l1_buffer + 1968;
  ...
  112, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  16, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 1576) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  256, // size: dimension of matrix of weight * bytes_per_weight
  16, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  16, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 1*1, 16, 1*1, 16, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 1576) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+256 + 128*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 1836 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+384 + 128*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 1968 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 1836 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 1968 + exec_db_act);
    W = (char *) (l1_buffer + 1576 + exec_db_W);
    y = (char *) (l1_buffer + 788 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 16 : 16;
