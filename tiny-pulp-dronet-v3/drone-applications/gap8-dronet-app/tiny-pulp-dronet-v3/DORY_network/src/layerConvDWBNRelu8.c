...
void layerConvDWBNRelu8(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+72;
    copy_k.loc = (uint32_t) l1_buffer + 2788;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 64;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+136;
    copy_lambda.loc = (uint32_t) l1_buffer + 2856;
  ...
  72, // size: dimension of matrix of weight * bytes_per_weight
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      {
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 8.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+72 + 64*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 2788 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+136 + 64*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 2856 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 2788 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 2856 + exec_db_act);
