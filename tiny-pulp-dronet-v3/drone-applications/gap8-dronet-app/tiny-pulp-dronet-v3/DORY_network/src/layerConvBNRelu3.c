...
void layerConvBNRelu3(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+16;
    copy_k.loc = (uint32_t) l1_buffer + 5028;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 32;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+48;
    copy_lambda.loc = (uint32_t) l1_buffer + 5064;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  2500, // size: dimension of the buffer
  100, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  4, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  25,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 5008) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  16, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_x = !db_state_x ? 2500 : 0;
    db_W = !db_state_W ? 16 : 0;
    db_y = !db_state_y ? 2500 : 0;
    db_act = !db_state_W ? 32 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 16 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 1*1, 4, 1*1, 4, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 5008) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+16 + 32*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 5028 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+48 + 32*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 5064 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 5028 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 5064 + exec_db_act);
    W = (char *) (l1_buffer + 5008 + exec_db_W);
    y = (char *) (l1_buffer + 2504 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 25 : 25;
