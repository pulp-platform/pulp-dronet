...
void layerConvBNRelu11(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+128;
    copy_k.loc = (uint32_t) l1_buffer + 1316;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 128;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+256;
    copy_lambda.loc = (uint32_t) l1_buffer + 1448;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  392, // size: dimension of the buffer
  56, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 1184) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_x = !db_state_x ? 392 : 0;
    db_W = !db_state_W ? 128 : 0;
    db_y = !db_state_y ? 784 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 1*1, 8, 1*1, 8, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 1184) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+128 + 128*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 1316 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+256 + 128*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 1448 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 1316 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 1448 + exec_db_act);
    W = (char *) (l1_buffer + 1184 + exec_db_W);
    y = (char *) (l1_buffer + 396 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 7 : 7;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 16 : 16;
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 7, 7, 16, 7, 16, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 396) + db_y, // loc
