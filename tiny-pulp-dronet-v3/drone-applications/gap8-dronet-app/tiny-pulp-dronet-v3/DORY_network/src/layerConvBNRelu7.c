...
void layerConvBNRelu7(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+32;
    copy_k.loc = (uint32_t) l1_buffer + 2072;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 64;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+96;
    copy_lambda.loc = (uint32_t) l1_buffer + 2140;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  676, // size: dimension of the buffer
  52, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 2036) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  32, // size: dimension of matrix of weight * bytes_per_weight
  4, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  4, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_x = !db_state_x ? 676 : 0;
    db_W = !db_state_W ? 32 : 0;
    db_y = !db_state_y ? 1352 : 0;
    db_act = !db_state_W ? 64 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 32 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 4, 1*1, 4, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 2036) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+32 + 64*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 2072 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+96 + 64*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 2140 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 2072 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 2140 + exec_db_act);
    W = (char *) (l1_buffer + 2036 + exec_db_W);
    y = (char *) (l1_buffer + 680 + db_y);
