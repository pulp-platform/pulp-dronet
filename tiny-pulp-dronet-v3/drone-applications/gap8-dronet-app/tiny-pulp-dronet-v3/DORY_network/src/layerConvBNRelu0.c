...
void layerConvBNRelu0(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+100;
    copy_k.loc = (uint32_t) l1_buffer + 26686;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 32;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+132;
    copy_lambda.loc = (uint32_t) l1_buffer + 26754;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  6965, // size: dimension of the buffer
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 26482) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  100, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<1*1*7*2; iter++) {
    ...
    db_x = !db_state_x ? 6965 : 0;
    db_W = !db_state_W ? 100 : 0;
    db_y = !db_state_y ? 6272 : 0;
    db_act = !db_state_W ? 32 : 0;
    exec_db_x = db_state_x ? 6965 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 100 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<1*1*7*2-1) 
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 6 : 199;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 2 : 98;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 35, 199, 1, 200, 1,  3, 3,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 5*5, 1, 5*5, 1, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 26482) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+100 + 32*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 26686 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+132 + 32*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 26754 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 26686 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 26754 + exec_db_act);
    W = (char *) (l1_buffer + 26482 + exec_db_W);
    y = (char *) (l1_buffer + 13934 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 6 : 199;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 4 : 4;
    y_tile_size_h   = (_i_h_exec+1 == 7)   ? 4 : 16;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 2 : 98;
    ...
    if(iter<1*1*7*2-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 98, 4, 100, 4, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 13934) + db_y, // loc
