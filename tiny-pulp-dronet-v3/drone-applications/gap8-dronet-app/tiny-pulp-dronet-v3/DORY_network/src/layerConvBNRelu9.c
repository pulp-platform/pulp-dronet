...
void layerConvBNRelu9(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 64;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+64;
    copy_k.loc = (uint32_t) l1_buffer + 2780;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 64;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+128;
    copy_lambda.loc = (uint32_t) l1_buffer + 2848;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  1352, // size: dimension of the buffer
  104, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  8, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  13,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  8, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 2712) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  64, // size: dimension of matrix of weight * bytes_per_weight
  8, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  8, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    db_x = !db_state_x ? 1352 : 0;
    db_W = !db_state_W ? 64 : 0;
    db_y = !db_state_y ? 1352 : 0;
    db_act = !db_state_W ? 64 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 64 : 0;
    exec_db_act = db_state_W ? 64 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter<1*1*1*1-1) 
    {
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 13 : 13;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 13 : 13;
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 8, 1*1, 8, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 2712) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+64 + 64*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 2780 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+128 + 64*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 2848 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 2780 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 2848 + exec_db_act);
    W = (char *) (l1_buffer + 2712 + exec_db_W);
    y = (char *) (l1_buffer + 1356 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 8 : 8;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 13 : 13;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 13 : 13;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 8 : 8;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 13 : 13;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 13 : 13;
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 13, 13, 8, 13, 8, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 1356) + db_y, // loc
